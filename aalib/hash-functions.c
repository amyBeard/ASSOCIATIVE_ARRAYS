...
HashIndex customHash(AAKeyType key, size_t keyLength, HashIndex size){
	HashIndex hash = 0;

	//custom hash is to get ascii value and square it
	for (size_t i = 0; i < keyLength; i++) {
	...
	)
{
    ...
    while (current >= 0 && current < hashTable->size) {
        if (hashTable->table[current].key == NULL ||
            (size_t)hashTable->table[current].key == HASH_DELETED) {
            return current;
        }
	...
	)
{
    ...
    while (count < hashTable->size) {
        if (hashTable->table[current].key == NULL || (size_t)hashTable->table[current].key == HASH_DELETED) {
            return current;
        }

        count++;
        current = (startIndex + count * count) % hashTable->size;
	...
	)
{
    ...
    HashIndex secondaryHash = hashTable->hashAlgorithmSecondary(key, keylen, hashTable->size);
    ...
    while (current >= 0 && current < hashTable->size) {
        ...
        if (hashTable->table[current].validity != HASH_USED || (size_t)hashTable->table[current].key == HASH_DELETED) {
			return current;
        }
     
        current = (current + secondaryHash) % hashTable->size;
