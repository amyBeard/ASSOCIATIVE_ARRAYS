...
aaDeleteAssociativeArray(AssociativeArray *aarray)
{
	 ...
	 */
	 if (aarray) {
		...
		if (aarray->hashNamePrimary) {
        free(aarray->hashNamePrimary);
		}
		if (aarray->hashNameSecondary) {
...
int aaInsert(AssociativeArray *aarray, AAKeyType key, size_t keylen, void *value)
{
    ...
    AAKeyType newKey = malloc(keylen);
    if (newKey == NULL) {
		free(newKey);
        return -1;
    }
    ...
    HashIndex hash = aarray->hashAlgorithmPrimary(newKey, keylen, aarray->size) % aarray->size;

    while (aarray->table[hash].validity == HASH_USED) {
        if (aarray->table[hash].keylen == keylen && memcmp(aarray->table[hash].key, newKey, keylen) == 0) {
            free(newKey);
            aarray->table[hash].value = value;
			aarray->insertCost++;
            return hash;
        }

        hash = aarray->hashProbe(aarray, key, keylen, hash, 0, &(aarray->insertCost));
    }

    aarray->table[hash].key = newKey;
    aarray->table[hash].keylen = keylen;
    aarray->table[hash].value = value;
    aarray->table[hash].validity = HASH_USED;
	...
	)
{
	...
	HashIndex hash = aarray->hashAlgorithmPrimary(key, keylen, aarray->size) % aarray->size;
	//printf("%ld\n", hash);
    ...
    while (count < aarray->size) {
		char * newKey = (char *)aarray->table[hash].key;
		if (newKey != NULL && key != NULL) {
			if (strstr((char *)key , (char *)newKey) != NULL){
				aarray->searchCost++;
				return aarray->table[hash].value;
			}

		}

		aarray->searchCost++;
        hash = (hash + step) % aarray->size;
		count++;
    }
	aarray->searchCost++;
...
void *aaDelete(AssociativeArray *aarray, AAKeyType key, size_t keylen)
{
    ...
    while (count < aarray->size) {
		...
		if (newKey != NULL && key != NULL) {
			if (strstr((char *)key , newKey) != NULL) {
				aarray->table[hash].validity = HASH_DELETED;
				aarray->nEntries--;
				aarray->deleteCost++;
				return aarray->table[hash].value;
			}
